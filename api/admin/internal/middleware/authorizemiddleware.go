...
type AuthorizeMiddleware struct {
	config.AuthConf
	*language.Loader
	authHandler func(info types.LoginUser)
}

func NewAuthorizeMiddleware(conf config.AuthConf, langLoader *language.Loader, authHandler func(info types.LoginUser)) *AuthorizeMiddleware {
